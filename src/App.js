...
const App = () => {
  ...
  const genres1=["All",... new Set(movies.map((movie)=>movie.genre))]
  const filteredMovies=movies.filter((movie)=>selectedGenre==="All"||movie.genre===selectedGenre)
  .sort((a,b)=>a.genre.localeCompare(b.genre)|| a.title.localeCompare(b.title));
  return (
    <div>
      <h1>Top 15 Movies of All Time</h1>
      <GenreFilter genres={genres1} onGenreSelect={setSelectedGenre}/>
